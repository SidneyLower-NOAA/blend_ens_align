SUBROUTINE lpm_mean_la(myproc,nprocs,nx,ny,ibgn,iend,jbgn,jend, &
...
IF( myproc == root ) THEN
      WRITE(6,'(a)') ' -- Entering loop over patches'
END IF
...
IF( myproc == root ) THEN
      WRITE(6,'(a)') ' -- Done. Gathering results to root'
END IF
