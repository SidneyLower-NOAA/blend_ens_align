...
! WRITE(6,'(a,i5)') 'Inside rshift2dgrd, myproc=',myproc!
  IF( myproc == root ) THEN 
      WRITE(6, '(a)') ' --- [shift_ops/rshift2dgrd] --- '
  END IF

  nzone=nizone(ipass)*njzone(ipass)
  IF(nzone > mxzone) THEN
    IF( myproc == root ) THEN
        WRITE(6,'(a)') ' WARNING:: Increase mxzone sent to rshift3d'
...
!
    ...
    IF( myproc == root ) THEN
          CALL CPU_TIME(cput2)
          WRITE(6,'(a,f10.2,a)') ' -- Shift vectors CPU time:',(cput2-cput1),' seconds'
    END IF
!
!-----------------------------------------------------------------------
!
...
!
  ...
  INTEGER, INTENT(IN)  :: minkdat,thresh_flag
  REAL, INTENT(IN)     :: slen
  REAL, INTENT(IN)     :: wgtvar,threshvar
...
!
        ...
        IF( ii <= iend .AND.  ii >= ibgn  .AND. jj <= jend .AND.  jj >= jbgn) THEN
          ...
          IF( ia <= iend .AND.  ia >= ibgn  .AND. ja <= jend .AND.  ja >= jbgn .AND. &
            ...
            npts=npts+1
            DO ivar=1,nvar
              IF (posdef == 1 .AND. fcsta(ia,ja,ivar) < threshvar .AND. fcstb(ib,jb,ivar) < threshvar) THEN
                kpts=kpts+1
              ENDIF
              vardiff = (fcsta(ia,ja,ivar) - fcstb(ib,jb,ivar))
              sumsqx = sumsqx + wgtvar*(vardiff*vardiff)
            END DO     
          END IF
        END IF
